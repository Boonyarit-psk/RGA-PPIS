...
USE_EFEATS = False  # True/False

if BASE_MODEL_TYPE == 'GCN':
    USE_EFEATS = False

MAP_CUTOFF = 14 # criteria for adjacency matrix
DIST_NORM = 15 # for divide in Pseudo-position

# INPUT_DIM
if ADD_NODEFEATS == 'all':  # add atom features and psepose embedding # dept xyz
    INPUT_DIM = 54 + 7 + 1 
...
HIDDEN_DIM = INPUT_DIM   #256  # hidden size of node features
DROPOUT = 0.1
ALPHA = 0.7
LAMBDA = 1.5
BATCH_SIZE = 1
...
# function for normalize adjacency matrix (def load_graph)
def normalize(mx):
...
# got Distance Matrix to create adjacency matrix
def cal_edges(sequence_name, radius=MAP_CUTOFF):  # to get the index of the edges
    ...
    mask = ((dist_matrix >= 0) * (dist_matrix <= radius)) # true * true/false (1,0)
    adjacency_matrix = mask.astype(int)
    radius_index_list = np.where(adjacency_matrix == 1) # got index array (low), array(col)
    radius_index_list = [list(nodes) for nodes in radius_index_list] # change to [listrow, listcol]
    return radius_index_list # position where have edge 

# like cal_edge + normalize(mx) return adjacency matrix
...
# Class for set data to model
class ProDataset(Dataset):
    def __init__(self, dataframe, radius=MAP_CUTOFF, dist=DIST_NORM, psepos_path='Feature/psepos/Train335_psepos_SC.pkl'):
        
        self.names = dataframe['ID'].values # name of protein
        self.sequences = dataframe['sequence'].values # protein 1D
        self.labels = dataframe['label'].values # 0,1
        # position of each amino acid (used R side)
    ...
    def __getitem__(self, index):
        ...
        nodes_num = len(sequence) # number of node from protein 1D
        pos = self.residue_psepos[sequence_name]
        reference_res_psepos = pos[0] # ref position is first amino acid
        pos = pos - reference_res_psepos # change pos[0] = position (0,0,0)
        pos = torch.from_numpy(pos)

        # get node feature (pssm+hmm+dssp)
        ...
        # creat graph
        G = dgl.DGLGraph()
        G.add_nodes(nodes_num) # number node from sequence 
        edge_feat = np.transpose(edge_feat, (1, 2, 0))
        edge_feat = edge_feat.squeeze(1)
        # got edge and edge feature
        self.add_edges_custom(G,
                              radius_index_list,
                              edge_feat
                              )
        
        adj_matrix = load_graph(sequence_name) # got normalize adjacency matrix
    ...
    def cal_edge_attr(self, index_list, pos):
        ...
        # find distance of position(x,y,z) node low and node col that have edge 
        distance = (pdist(pos[index_list[0]], pos[index_list[1]]) / self.radius).detach().numpy()
        # result cosine is (-1,1) so +1 = (0,2) /2 = (0,1)
    ...
    def add_edges_custom(self, G, radius_index_list, edge_features):
        # position cal, position row (have edge)
        ...
        if len(src) != len(dst):
            ...
            raise Exception
        G.add_edges(src, dst) # source, destination
        G.edata['ex'] = torch.tensor(edge_features) # add edge data 'ex' = name
...
class GraphAttentionLayer(nn.Module):
    def __init__(self, in_features: int, out_features: int, n_heads: int, num_hops=4):
        ...
        # Parameters for transformation and attention
    ...
    def forward(self, h: torch.Tensor, adj_mat: torch.Tensor, pos: torch.Tensor):
        ...
        # Apply layer normalization after aggregation
        h_prime = self.layer_norm(h_prime.mean(dim=0))  # Mean over heads

        # Residual connection
        h_prime = h + h_prime

        # Position Update using Weighted Sum of Attention Scores
    ...
    def _get_attention_scores(self, h_transformed: torch.Tensor, pos: torch.Tensor):#, adj_mat: torch.Tensor):
        ...
        #Compute distance-based scores with learnable weight
